...
const opentelemetry::nostd::shared_ptr<opentelemetry::trace::Tracer> Tracer_RGW::noop_tracer = opentelemetry::trace::Provider::GetTracerProvider()->GetTracer("no-op", OPENTELEMETRY_SDK_VERSION);

using bufferlist = ceph::buffer::list;

Tracer_RGW::Tracer(opentelemetry::nostd::string_view service_name) {
  init(service_name);
}

void Tracer_RGW::init(opentelemetry::nostd::string_view service_name) {
  if (!tracer) {
...
void Tracer_RGW::shutdown() {
  if (tracer) {
    tracer->CloseWithMicroseconds(1);
  }
}

jspan Tracer_RGW::start_trace(opentelemetry::nostd::string_view trace_name) {
  if (is_enabled()) {
...
jspan Tracer_RGW::add_span(opentelemetry::nostd::string_view span_name, const jspan& parent_span) {
...
jspan Tracer_RGW::add_span(opentelemetry::nostd::string_view span_name, const jspan_context& parent_ctx) {
...
bool Tracer_RGW::is_enabled() const {
  return g_ceph_context->_conf->jaeger_tracing_enable;
}

void encode_RGW(const jspan_context& span_ctx, bufferlist& bl, uint64_t f) {
  ENCODE_START(1, 1, bl);
...
void decode_RGW(jspan_context& span_ctx, bufferlist::const_iterator& bl) {
  using namespace opentelemetry;
  using namespace trace;
  DECODE_START(1, bl);
  bool is_valid;
