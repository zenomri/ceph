...
class Tracer_RGW {
 ...
 public:
  Tracer_RGW() = default;
  Tracer_RGW(opentelemetry::nostd::string_view service_name);
...
void encode_RGW(const jspan_context& span, ceph::buffer::list& bl, uint64_t f = 0);
void decode_RGW(jspan_context& span_ctx, ceph::buffer::list::const_iterator& bl);
...
struct Tracer_RGW {
  ...
  inline void encode_RGW(const jspan_context& span, bufferlist& bl, uint64_t f=0) {}
  inline void decode_RGW(jspan_context& span_ctx, ceph::buffer::list::const_iterator& bl) {}
}
